...
# setarile vizuale pentru panoul de control
ctk.set_appearance_mode("dark")
ctk.set_default_color_theme("blue")

# incarca datele din .env
load_dotenv()
...
def execute_remote_command(command):
    """executa o comanda prin SSH si returneaza output-ul, fara sa blocheze executia"""
    try:
...
class CommandHub(ctk.CTk):
    def __init__(self):
        ...
        # --- zona dreapta (workspace + terminale) ---
    ...
    def _async_stop_monitor(self):
        ...
        raspuns = execute_remote_command(cmd) # acum prindem ce scuipa comanda kill
        # ii dam timp monitorului sa scrie in log si lui tail -f sa citeasca
        time.sleep(0.6)
        ...
        # daca monitorul a zis ceva de adio, il aruncam pe ecran
    ...
    # --- logica vizualizare baza de date (--list) ---
    ...
    def _async_fetch_scores(self, selected_districts):
        # transformi lista Python ['A', 'B'] intr-un string pur de C: "A B"
        districte_curate = " ".join(selected_districts)

        # acum injectezi string-ul curat in comanda
        cmd = f"cd {PROJECT_PATH} && echo 'calculate_scores {districte_curate}' | ./city_hub"
        raspuns = execute_remote_command(cmd)

        # randare in interfata
